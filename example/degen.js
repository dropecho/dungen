...
de_polygonal_ds_BinaryTreeNode.prototype = {
	preorder: function(process,iterative,userData) {
		...
		if(iterative == false) {
			if(process == null) {
				var run = this.val.visit(false,userData);
	...
	,inorder: function(process,iterative,userData) {
		...
		if(iterative == false) {
			if(process == null) {
				...
				if(!this.val.visit(false,userData)) {
					return;
				}
				if(this.right != null) {
	...
	,toString: function() {
		var b_b = "";
		var tmp = "[ BinaryTree val=" + Std.string(this.val) + " size=" + this.get_size() + " depth=";
		var node = this.parent;
		var c = 0;
		while(node != null) {
			node = node.parent;
		...
		b_b = b_b + Std.string(tmp + c + " height=" + this.height());
		if(this.get_size() == 1) {
			b_b = b_b + " ]";
			return b_b;
		}
		b_b = b_b + "\n";
		this.preorder(function(node1,userData) {
			...
			var t = "";
			var _g1 = 0;
			while(_g1 < d) if(_g1++ == d - 1) {
				t += ((node1.parent == null?false:node1.parent.left == node1)?"L":"R") + "---";
			} else {
				t += "|   ";
			}
			t = "  " + t;
			b_b = b_b + Std.string(t + Std.string(node1.val) + "\n");
			return true;
		});
		b_b = b_b + "]";
		return b_b;
	...
	,preorderRecursiveVisitable: function(node,userData) {
		var run = node.val.visit(false,userData);
		if(run && node.left != null) {
	...
	,inorderRecursiveVisitable: function(node,userData) {
		...
		if(!node.val.visit(false,userData)) {
			return false;
		}
		if(node.right != null) {
	...
	,postorderRecursiveVisitable: function(node,userData) {
		...
		return node.val.visit(false,userData);
	}
...
degen_bsp_Generator.prototype = {
	generate: function(root) {
		if(root == null) {
			root = new de_polygonal_ds_BinaryTreeNode(new degen_bsp_BspData({ height : this.height, width : this.width, x : this.x, y : this.y}));
		}
		this.buildTree(root);
		return root;
	}
	...
	,makeSplit: function(node) {
		...
		var splitAt = 0;
		var vertical;
		if(val.height < 2 * this.minHeight || val.width > val.height * this.ratio) {
			vertical = false;
		} else if(val.width < 2 * this.minWidth || val.height > val.width * this.ratio) {
			vertical = true;
		} else {
			return;
		}
		if(vertical) {
			splitAt = Std.random(val.height - this.minHeight * 2) + this.minHeight;
			...
			splitAt = Std.random(val.width - this.minWidth * 2) + this.minWidth;
...
degen_ca_Generator.buildFromCA = function(map,params,step) {
	...
	while(tmp.hasNext()) {
		var i = tmp.next();
		var pos1 = map.IndexToXY(i);
		map.set(pos1.x,pos1.y,temp.h[i]);
	}
};
...
degen_export_TiledExporter["export"] = function(filename,map) {
	return JSON.stringify(new degen_export__$TiledExporter_TiledMap(map),null," ");
};
...
degen_map_Map2d.prototype = {
	initializeData: function(initTileData) {
		...
		while(_g1 < _g) this._mapData[_g1++] = initTileData;
	}
	,fillMapRandomly: function(wallTile,floorTile,wallPercent) {
		...
		while(_g1 < _g) this._mapData[_g1++] = Std.random(100) > wallPercent?floorTile:wallTile;
	}
	...
	,toString: function() {
		...
		while(_g1 < _g) {
			...
			var _g3 = 0;
			var _g2 = this._width;
			while(_g3 < _g2) output += String.fromCharCode(this._mapData[this.XYtoIndex(_g3++,y)]);
			output += "\n";
		}
		return output;
	}
...
degen_map_MapHelper.isMapConnected = function(map,tile,diagonal) {
	...
	return degen_map_MapHelper.getFirstEmptyTile(map,tile,degen_map_MapHelper.floodFill(map,start.x,start.y,tile,diagonal)) == null;
};
degen_map_MapHelper.getFirstEmptyTile = function(map,tile,ignore) {
	...
	while(_g1 < _g) {
		...
		if(map._mapData[i] == tile) {
			...
			if(ignore != null) {
				if(Lambda.find(ignore,(function(cur1) {
...
degen_map_MapHelper.floodFill = function(map,startX,startY,tile,diagonal) {
	...
	while(open.length > 0) {
		...
		var key1 = map.XYtoIndex(currentTile.x,currentTile.y);
		closed.h[key1] = currentTile;
...
degen_map_MapHelper.getHallwayTiles = function(map,tile) {
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			var tmp = map.getNeighborCount(i,j,tile,2) < 8;
		}
	}
	return hallwayTiles;
};
...
degen_map_generators_MixedGenerator.buildRooms = function(root,userData) {
	...
	var makeRooms = function(node,foo) {
		...
		while(_g1 < roomEndX) {
			var x = _g1++;
			var _g3 = roomStartY;
			while(_g3 < roomEndY) map.set(x,_g3++,userData.tile_floor);
		}
		return true;
	};
	var makeCaveFromCA = function(node1,userData1) {
		...
		while(_g11 < _g) {
			var x1 = _g11++;
			var _g31 = 0;
			var _g2 = cave._height;
			while(_g31 < _g2) {
				var y = _g31++;
				map.set(x1 + roomStartX1,y + roomStartY1,cave.get(x1,y));
			}
		}
		return true;
	};
	var makeCorridors = function(node2,userData2) {
		...
		var _g12 = startX;
		var _g4 = endX;
		while(_g12 < _g4) map.set(_g12++,startY,userData2.tile_floor);
		var _g13 = startY;
		var _g5 = endY;
		while(_g13 < _g5) map.set(startX,_g13++,userData2.tile_floor);
		return true;
	};
	...
	var closeEdges = function(node4,userData4) {
		...
		var _g14 = 0;
		var _g6 = node4.val.width;
		while(_g14 < _g6) {
			var x2 = _g14++;
			map.set(x2,0,userData4.tile_wall);
			map.set(x2,node4.val.height,userData4.tile_wall);
		}
		var _g15 = 0;
		var _g7 = node4.val.height;
		while(_g15 < _g7) {
			var y1 = _g15++;
			map.set(0,y1,userData4.tile_wall);
			map.set(node4.val.width,y1,userData4.tile_wall);
		}
		return false;
	};
...
degen_map_generators_RoomGenerator.buildRooms = function(root,params) {
	...
	var makeRoom = function(node,foo) {
		...
		while(_g1 < roomEndX) {
			var x = _g1++;
			var _g3 = roomStartY;
			while(_g3 < roomEndY) map.set(x,_g3++,params.tileFloor);
		}
		return true;
	};
	var makeCorridors = function(node1,userData) {
		...
		var _g11 = startX;
		var _g = endX;
		while(_g11 < _g) map.set(_g11++,startY,params.tileFloor);
		var _g12 = startY;
		var _g2 = endY;
		while(_g12 < _g2) map.set(startX,_g12++,params.tileFloor);
		return true;
	};
...
degen_map_helpers_ConnectivityChecker.prototype = {
	check: function(map,tile) {
		...
		degen_map_MapHelper.floodFill(map,tile,firstEmpty.x,firstEmpty.y);

...
dropecho_ds_Graph.prototype = {
	...
	,addNode: function(node) {
		dropecho_interop_AbstractMap.set(this.nodes,node.id,node);
		node.graph = this;
		return node;
	}
	,addUniEdge: function(nodeId,otherId,data) {
		if(dropecho_interop_AbstractMap.exists(this.edges,nodeId)) {
			dropecho_interop_AbstractMap.set(dropecho_interop_AbstractMap.get(this.edges,nodeId),otherId,data);
		} else {
			var map = dropecho_interop_AbstractMap._new();
			dropecho_interop_AbstractMap.set(map,otherId,data);
			dropecho_interop_AbstractMap.set(this.edges,nodeId,map);
		}
	}
	...
	,remove: function(id) {
		var _g = 0;
		var _g1 = this.inNeighborIds(dropecho_interop_AbstractMap.get(this.nodes,id));
		while(_g < _g1.length) {
			var n = _g1[_g];
			++_g;
			dropecho_interop_AbstractMap.remove(dropecho_interop_AbstractMap.get(this.edges,n),id);
		}
		dropecho_interop_AbstractMap.remove(this.edges,id);
		dropecho_interop_AbstractMap.remove(this.nodes,id);
	}
	,inNeighbors: function(node,filter) {
		...
		while(_g < _g1) {
			var i = _g++;
			result[i] = dropecho_interop_AbstractMap.get(_gthis.nodes,_this[i]);
		}
		return result;
	}
	,inNeighborIds: function(node,filter) {
		var _g = [];
		var jsIterator = this.edges.entries();
		...
		while(!_g1_lastStep.done) {
			...
			var id = _g1[0];
			var edge = _g1[1];
			if(dropecho_interop_AbstractMap.exists(edge,node.id) && (filter == null || filter(id,dropecho_interop_AbstractMap.get(edge,node.id)))) {
				_g.push(id);
			}
		}
		var ids = _g;
	...
	,outNeighborIds: function(node,filter) {
		if(!dropecho_interop_AbstractMap.exists(this.edges,node.id)) {
			return [];
		}
		var _g = [];
		var jsIterator = dropecho_interop_AbstractMap.get(this.edges,node.id).entries();
		var _g1_jsIterator = jsIterator;
		var _g1_lastStep = jsIterator.next();
		while(!_g1_lastStep.done) {
			var v = _g1_lastStep.value;
	...
	,edgeData: function(fromId,toId) {
		if(dropecho_interop_AbstractMap.exists(this.edges,fromId)) {
			return dropecho_interop_AbstractMap.get(dropecho_interop_AbstractMap.get(this.edges,fromId),toId);
		}
		return null;
	}
	,toString: function() {
		...
		var jsIterator = this.nodes.values();
...
var dropecho_ds_Heap = $hx_exports["Heap"] = function(compare) {
	this.compare = compare != null ? compare : function(a,b) {
		return Reflect.compare(a,b) < 0;
	};
	this.elements = [];
};
dropecho_ds_Heap.__name__ = "dropecho.ds.Heap";
dropecho_ds_Heap.prototype = {
	compare: null
	,elements: null
	,push: function(n) {
		this.set_value(this.elements.push(n) - 1,n);
	}
	,set_value_obj: function(oldVal,newVal) {
		this.set_value(this.elements.indexOf(oldVal),newVal);
	}
	,set_value: function(i,val) {
		this.elements[i] = val;
	...
	,_getLeft: function(index) {
		return 2 * index + 1;
	}
	,_getRight: function(index) {
		return 2 * index + 2;
	}
	,__class__: dropecho_ds_Heap
};
...
var dropecho_ds_graph_NodeDist = function(node,dist) {
	this.node = node;
	this.dist = dist;
};
dropecho_ds_graph_NodeDist.__name__ = "dropecho.ds.graph.NodeDist";
dropecho_ds_graph_NodeDist.prototype = {
	node: null
	,dist: null
	,__class__: dropecho_ds_graph_NodeDist
};
var dropecho_ds_graph_SearchResult = function(path,distances) {
	this.path = path;
	this.distances = distances;
};
dropecho_ds_graph_SearchResult.__name__ = "dropecho.ds.graph.SearchResult";
dropecho_ds_graph_SearchResult.prototype = {
	path: null
	,distances: null
	,__class__: dropecho_ds_graph_SearchResult
};
var dropecho_ds_graph_Search = $hx_exports["graph"]["Search"] = function() { };
dropecho_ds_graph_Search.__name__ = "dropecho.ds.graph.Search";
dropecho_ds_graph_Search.dijkstra = function(graph,node,distCalc) {
	var compare = function(a,b) {
		return Reflect.compare(a.dist,b.dist) < 0;
	};
	var queue = new dropecho_ds_Heap(compare);
	var dist = dropecho_interop_AbstractMap._new();
	var prev = dropecho_interop_AbstractMap._new();
	if(distCalc == null) {
		distCalc = function(a,b) {
			return 1;
		};
	}
	dropecho_interop_AbstractMap.set(dist,node,0.0);
	var jsIterator = graph.nodes.values();
	var _g_jsIterator = jsIterator;
	var _g_lastStep = jsIterator.next();
	while(!_g_lastStep.done) {
		var v = _g_lastStep.value;
		...
		var n = v;
		if(n != node) {
			dropecho_interop_AbstractMap.set(dist,n,Infinity);
			dropecho_interop_AbstractMap.set(prev,n,null);
		}
		queue.push(new dropecho_ds_graph_NodeDist(n,dropecho_interop_AbstractMap.get(dist,n)));
	}
	while(queue.size() > 0) {
		var minDistNode = queue.pop().node;
		var _this = queue.elements;
		var result = new Array(_this.length);
		var _g = 0;
		var _g1 = _this.length;
		while(_g < _g1) {
		...
		var filter = (function(existingIds) {
			return function(id,data) {
				return existingIds[0].indexOf(id) >= 0;
			};
		})(existingIds);
		var neighbors = graph.neighbors(minDistNode,filter);
		var _g2 = 0;
		while(_g2 < neighbors.length) {
			var neighbor = [neighbors[_g2]];
			++_g2;
			var distanceToNeighbor = dropecho_interop_AbstractMap.get(dist,minDistNode) + distCalc(minDistNode,neighbor[0]);
			if(distanceToNeighbor <= dropecho_interop_AbstractMap.get(dist,neighbor[0])) {
				dropecho_interop_AbstractMap.set(dist,neighbor[0],distanceToNeighbor);
				dropecho_interop_AbstractMap.set(prev,neighbor[0],minDistNode.id);
				var existing = Lambda.find(queue.elements,(function(neighbor) {
					return function(x) {
						return x.node == neighbor[0];
					};
				})(neighbor));
				queue.set_value_obj(existing,new dropecho_ds_graph_NodeDist(neighbor[0],dropecho_interop_AbstractMap.get(dist,neighbor[0])));
			}
		}
	}
	return new dropecho_ds_graph_SearchResult(prev,dist);
};
var dropecho_ds_graph_Traversal = $hx_exports["graph"]["traversal"] = function() { };
dropecho_ds_graph_Traversal.__name__ = "dropecho.ds.graph.Traversal";
dropecho_ds_graph_Traversal.breadthFirst = function(node) {
...
dropecho_ds_graph_Traversal.depthFirst = function(node) {
	var graph = node.graph;
	var visited = [];
	...
	var _g = 0;
	var _g1 = graph.neighbors(node);
	while(_g < _g1.length) {
		var n = _g1[_g];
		++_g;
		dropecho_ds_graph_Traversal.depthFirst(n);
	}
	return visited;
};
...
dropecho_dungen_map_extensions_FloodFill.floodFill = function(map,startX,startY,tile,diagonal) {
	...
	while(open.length > 0) {
		...
		var _g = [];
		var _g1 = 0;
		var _g2 = dropecho_dungen_map_extensions_Neighbors.getNeighbors(map,currentTile.x,currentTile.y,1,diagonal);
...
dropecho_interop_AbstractMap.fromMap = function(map) {
	return dropecho_interop_AbstractMap._new(new Map(map));
};
dropecho_interop_AbstractMap.fromIMap = function(map) {
	return dropecho_interop_AbstractMap._new(new Map(map));
};
dropecho_interop_AbstractMap.remove = function(this1,key) {
...
dropecho_interop_Extender.defaults = function(base,extension) {
	if(base == null) {
		haxe_Log.trace("extender base is null",{ fileName : "dropecho/interop/Extender.hx", lineNumber : 21, className : "dropecho.interop.Extender", methodName : "defaults", customParams : [base == null]});
	...
	while(_g < extensions.length) {
		...
		while(_g3 < fields.length) {
			...
			if(bfIsArray) {
				...
				var copy1 = dropecho_interop_AbstractMap.fromMap(exField);
				var jsIterator = copy1.entries();
				var _g_jsIterator = jsIterator;
				var _g_lastStep = jsIterator.next();
				while(!_g_lastStep.done) {
					var v1 = _g_lastStep.value;
					_g_lastStep = _g_jsIterator.next();
					var _g6 = v1;
					var k = _g6[0];
					var v2 = _g6[1];
				...
				try {
					...
					var ex1 = haxe_Exception.caught(_g7).unwrap();
					haxe_Log.trace("FAILED SETTING PROP: " + ff + " error: " + Std.string(ex1),{ fileName : "dropecho/interop/Extender.hx", lineNumber : 86, className : "dropecho.interop.Extender", methodName : "defaults"});
				}
			}
		}
	}

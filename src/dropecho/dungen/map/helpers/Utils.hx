...
class Utils {
	...
	static function checkOverlap(r1 : Rect, r2: Rect) : Bool {
	...
	static function contains(r1:Rect, r2:Rect) : Bool {
		var r1p1 = {x: r1.x, y: r1.y};
		var r1p2 = {x: r1.x + r1.width, y: r1.y + r1.height};
		var r2p1 = {x: r2.x, y: r2.y};
		var r2p2 = {x: r2.x + r2.width, y: r2.y + r2.height};
		...
		return (r2p2.x < r1p2.x &&
		        r2p2.y < r1p2.y &&
		        r2p1.x > r1p1.x &&
		        r2p1.y > r1p1.y);
	}

  static function isOverlappingArray(r1, a:Array<Dynamic>) {
    for(r in a){
      if(r == r1){
	...
	static function shuffleArray<T>(a:Array<T>) : Array<T> {
		var rand:T;
		var shuffled = new Array<T>();
		var copy = a.slice(0);

		while(copy.length > 0) {
			rand = copy.splice(Std.random() * copy.length, 1);
			shuffled.push(rand);
		}

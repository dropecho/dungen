    ...
    for (i in 0...100) {
      ...
      root.inorder(function(node: BinaryTreeNode<BspData>, foo: Dynamic) : Bool{
        if (node.hasLeft() && node.hasRight()) {
          //vertical split
          if (node.left.val.height == node.val.height) {
            var childWidths = node.left.val.width + node.right.val.width;

            Assert.areEqual(node.val.width, childWidths);
            Assert.areEqual(node.val.height, node.right.val.height);
            Assert.areEqual(node.val.height, node.left.val.height);
          } else {
            var childHeights = node.left.val.height + node.right.val.height;
            Assert.areEqual(node.val.height, childHeights);
            Assert.areEqual(node.val.width, node.left.val.width);
            Assert.areEqual(node.val.width, node.right.val.width);
          }
        }
        return true;
      });
